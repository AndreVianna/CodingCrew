...
def read_lines(interrupt: bool = True,
    ...
    max_line_size = __get_screen_size()
    multiline = list[str]()
    line = ""
    ...
    try:
        while True:
            ...
            if key in exit_keys:
                __handle_exit(line, multiline)
                break
            elif key in linebreak_keys or key in exit_keys:
                line = __handle_linebreak(line, multiline)
            elif key == Key.BACKSPACE:
                line = __handle_backspace(line, multiline, max_line_size)
            elif key.isprintable():
                line = __handle_character(line, key)
    finally:
        __end_read(fd, old_settings)

    if indentation_mode == IndentationMode.REMOVE_ALL:
        multiline = [line.lstrip() for line in multiline]
    elif indentation_mode in (IndentationMode.UNINDENT, IndentationMode.NORMALIZE) and len(multiline) > 0:
        multiline = __handle_unindent(indentation_mode, indent_size, indent_str, multiline)
    if trim_line_ends:
...
def __handle_exit(line: str, multiline: list[str]):
    multiline.append(line)
    __write("\n")

def __handle_character(line: str, key: str) -> str:
    line += key
    __write(key)
    return line

def __handle_linebreak(line: str, multiline: list[str]) -> str:
    multiline.append(line)
    __write("\n")
    return ""

def __handle_backspace(line: str, multiline: list[str], max_line_size: int) -> str:
    ...
    if line:
        ...
        line = line[:-1]
        line_count = len(line) // max_line_size
...
def __handle_unindent(indentation_mode, indent_size, indent_str, multiline):
    offset = min(len(line) - len(line.lstrip()) for line in multiline if line.strip())
    result = list[str]()
